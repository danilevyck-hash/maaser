...
export default function ExpenseModal({ isOpen, onClose, onSave, editingExpense, saving }: Props) {
  ...
  const inputClass = "w-full border border-[#E5E5EA] rounded-[14px] px-4 py-3 text-[15px] focus:border-[#007AFF] outline-none bg-white";
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col h-full">
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1" style={{ WebkitOverflowScrolling: "touch" }}>
          <div>
            <label className="block text-[14px] font-medium text-[#6E6E73] mb-1.5">Monto ($)</label>
            <input
            ...
            />
            {amountError && <p className="text-[#FF3B30] text-[14px] mt-1">{amountError}</p>}
          </div>
          <div>
            <label className="block text-[14px] font-medium text-[#6E6E73] mb-1.5">Fecha</label>
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={inputClass} required />
          </div>
          <div>
            <label className="block text-[14px] font-medium text-[#6E6E73] mb-1.5">Notas</label>
