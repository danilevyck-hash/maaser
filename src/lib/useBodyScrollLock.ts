...
export function useBodyScrollLock(isOpen: boolean) {
  useEffect(() => {
    ...
    // Lock: prevent all scrolling
    html.style.overflow = "hidden";
    body.style.overflow = "hidden";
    body.style.touchAction = "none";

    // On iOS, also prevent the scroll position from jumping
    // by keeping the document at the same scroll position
    // (overflow:hidden alone can reset scroll to 0)
