...
export default function NuevoContrato() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (renewId) {
        ...
        setForm((f) => ({
          ...f,
          property_id: String(avail[0].id),
          rent_amount: String(avail[0].rent_amount),
        }));
      }
  ...
  async function handleSave() {
    ...
    // If renewing, deactivate old contract
    if (renewContract) {
      await fetch("/api/propiedades/contracts", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: renewContract.id, active: false }),
      });
    }
  ...
  // For property select: when renewing show all props, otherwise only available
      ...
      <div className="bg-white border-b border-gray-200 px-5 py-4 shrink-0">
        <div className="flex items-center gap-3 max-w-[430px] mx-auto">
          ...
          <h1 className="text-base font-semibold text-gray-900">
            {renewContract ? "Renovar contrato" : "Nuevo contrato"}
          </h1>
        </div>
      </div>
